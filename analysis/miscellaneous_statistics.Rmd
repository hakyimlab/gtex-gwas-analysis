...
gwas_metadata <- gtex_gwas_metadata() %>% select(Tag, new_Phenotype, Category) %>% rename(phenotype="Tag")
counts.per.ldblock <- counts.per.ldblock %>% inner_join(gwas_metadata %>% select(phenotype, Category))
...
```{r fig1, fig.width=10, fig.asp=0.8, echo=FALSE}
GWAS_COUNT_THR <- 10 # discard traits with less than this number of GWAS hits
pp <- ggplot() + 
  geom_point(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), mapping = aes(x = new_Phenotype, y = gwas_counts, col="GWAS"), size=1.5) +
  geom_line(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), mapping = aes(as.numeric(abbreviation)-min(as.numeric(abbreviation))+1, gwas_counts, col="GWAS"), group=1) +
  geom_bar(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), mapping = aes(x = new_Phenotype, y = gwas_mx_counts, fill="MultiXcan"), stat="identity") +
  geom_bar(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), mapping = aes(x = new_Phenotype, y = gwas_mx_enloc_counts, fill="MultiXcan + Enloc"), stat="identity") +  
...
```{r fig2, fig.width=10, fig.asp=1.5, echo=FALSE}
pp <- ggplot() + 
  geom_point(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), mapping = aes(x = new_Phenotype, y = gwas_counts, col="GWAS"), size=1.5) +
  geom_bar(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), mapping = aes(x = new_Phenotype, y = gwas_mx_counts, fill="MultiXcan"), stat="identity") +
  geom_bar(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), mapping = aes(x = new_Phenotype, y = gwas_mx_enloc_counts, fill="MultiXcan + Enloc"), stat="identity") +  
...
tissue_metadata <- get_gtex_tissue_metadata() %>% select(tissue, v8_all, tissue_abbrv) %>% rename(sample_size=v8_all)

df <- inner_join(t.test_df, tissue_metadata, by="tissue") %>% inner_join(gwas_metadata, by="phenotype")
...
```{r fig3, fig.width = 10, fig.asp = 0.7}
...
```{r fig4, fig.width = 10, fig.asp = 1.8}
ggp1 <- ggp1 + facet_grid(Category ~ .)
ggp1
```

```{r fig5, fig.width = 10, fig.asp = 0.7}
...
ggp2 <- ggplot(df, aes(x=new_Phenotype, y=-log10(t.test_pvalue))) + 
        geom_boxplot() + 
        theme_bw() + theme(axis.text.x = element_text(angle = 60, hjust = 1))
ggp2
```

```{r fig6, fig.width = 10, fig.asp = 2}
ggp2 <- ggp2 + facet_wrap(Category ~ ., drop = T, scales="free_x", nrow = 3)
ggp2
```
