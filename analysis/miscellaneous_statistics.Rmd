...
gwas_metadata <- gtex_gwas_metadata() %>% select(Tag, new_Phenotype) %>% rename(phenotype="Tag")

df <- inner_join(t.test_df, tissue_metadata, by="tissue")
df <- inner_join(df, gwas_metadata, by="phenotype")
...
ggp1 <- ggplot(df, aes(x=tissue, y=-log10(t.test_pvalue))) + geom_boxplot() + theme_bw() + theme(axis.text.x = element_text(angle = 60, hjust = 1))
ggp1
...
ggp2 <- ggplot(df, aes(x=new_Phenotype, y=-log10(t.test_pvalue))) + geom_boxplot() + theme_bw() + theme(axis.text.x = element_text(angle = 60, hjust = 1))
ggp2
```