...
```{r fig1, fig.width=10, fig.asp=0.8}
GWAS_COUNT_THR <- 10
pp <- ggplot() + 
  geom_point(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), 
             mapping = aes(x = new_Phenotype, y = gwas_counts, col="GWAS"), size=1.5) +
  geom_line(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), mapping = aes(as.numeric(abbreviation)-min(as.numeric(abbreviation))+1, 
                          gwas_counts, col="GWAS"), group=1) +
  geom_bar(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR),
           mapping = aes(x = new_Phenotype, y = gwas_mx_counts, fill="MultiXcan"), stat="identity") +
  geom_bar(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR),
           mapping = aes(x = new_Phenotype, y = gwas_mx_enloc_counts, fill="MultiXcan + Enloc"), stat="identity") +  
...
```{r fig2, fig.width=10, fig.asp=1.5}
pp <- ggplot() + 
  geom_point(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR), 
             mapping = aes(x = new_Phenotype, y = gwas_counts, col="GWAS"), size=1.5) +
  geom_bar(data = counts.per.ldblock %>% filter(gwas_counts > GWAS_COUNT_THR),
  ...
  scale_fill_manual(name="Method", values=cols) + # +
  scale_color_manual(label="", values=cols)# +
pp <- pp + facet_wrap(Category ~ ., drop = T, scales="free_x", nrow = 3)
pp
```
...
```{r fig3, fig.width = 10, fig.asp = 1}
ggp1 <- ggplot(df, aes(x=tissue, y=-log10(t.test_pvalue))) + 
        geom_boxplot() + theme_bw() + theme(axis.text.x = element_text(angle = 60, hjust = 1))
ggp1
```
