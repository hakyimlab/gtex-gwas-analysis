...
- `r n_models_genes_eqtl_eur` genes in Elastic Expression models
...
sp_eqtl_assoc_df <- sp_table_(predixcan_tbl_eqtl, predixcan_tbl_count_eqtl, "gene", "gene_name, ")
sp_sqtl_assoc_df <- sp_table_(predixcan_tbl_sqtl, predixcan_tbl_count_sqtl, "gene", "gene_name, ")
...
sm_eqtl_assoc_table <- sm_table_(multixcan_tbl_eqtl, multixcan_tbl_count_eqtl, "gene", "gene_name, ")
sm_sqtl_assoc_table <- sm_table_(multixcan_tbl_sqtl, multixcan_tbl_count_sqtl, "gene", "gene_name, ")
...
eqtl_enloc_df <- e_table_(enloc_tbl_eqtl_eur)
sqtl_enloc_df <- e_table_(enloc_tbl_sqtl_eur)

c_table_ <- function(c_tbl) {
    query <- "
SELECT phenotype, tissue, gene_id as gene, pp_H4_abf as p4
FROM {c_tbl$dataset_name}.{c_tbl$table_name} 
WHERE pp_H4_abf > 0.5 AND phenotype in {pheno_whitelist}
ORDER BY phenotype, tissue, pp_H4_abf DESC 
" %>% glue::glue()
  #print(query)
 query_exec(query, project = "gtex-awg-im", use_legacy_sql = FALSE, max_pages = Inf)
}
eqtl_coloc_df <- c_table_(coloc_tbl_eqtl )
...
sp_eqtl_discovery_table <- sp_table(predixcan_tbl_eqtl, predixcan_tbl_count_eqtl, enloc_tbl_eqtl_eur, "gene", "gene_name, ")
...
sp_sqtl_discovery_table <- sp_table(predixcan_tbl_sqtl, predixcan_tbl_count_sqtl, enloc_tbl_sqtl_eur, "gene", "gene_name, ")
sp_sqtl_discovery_table %>% rename(intron=gene) %>% select(-gene_name) %>% save_delim(fp_("spredixcan_enloc_discovery_sqtl.txt"))
#gsutil cp -a public-read output/spredixcan_enloc_discovery_sqtl.txt gs://gtex-gwas-share/data-for-paper/spredixcan_enloc_discovery_sqtl.txt
...
sm_eqtl_discovery_table <- sm_table(multixcan_tbl_eqtl, multixcan_tbl_count_eqtl, enloc_tbl_eqtl_eur, "gene", "gene_name, ")
sm_eqtl_discovery_table %>% save_delim( fp_("smultixcan_enloc_discovery_eqtl.txt"))
#gsutil cp -a public-read output/smultixcan_enloc_discovery_eqtl.txt gs://gtex-gwas-share/data-for-paper/smultixcan_enloc_discovery_eqtl.txt

sm_sqtl_discovery_table <- sm_table(multixcan_tbl_sqtl, multixcan_tbl_count_sqtl, enloc_tbl_sqtl_eur, "gene", "gene_name, ")
sm_sqtl_discovery_table %>% rename(intron=gene) %>% select(-gene_name) %>% save_delim( fp_("smultixcan_enloc_discovery_sqtl.txt"))
#gsutil cp -a public-read output/smultixcan_enloc_discovery_sqtl.txt gs://gtex-gwas-share/data-for-paper/smultixcan_enloc_discovery_sqtl.txt
