...
for (rcp_threshold in args$rcp_thresholds) {
  dd_rcp <- dd_wide %>% filter(rcp > rcp_threshold)
  rows <- c(
    rows, 
    list(
      data.frame(
        ...
        "ld_1st_quartile"=quantile(dd_rcp$r2, 0.25, na.rm=T),
        "ld_3rd_quartile"=quantile(dd_rcp$r2, 0.75, na.rm=T)
      )
    )
  )
}
...
write.table(output_df, file = output_filename, quote = FALSE, sep = "\t", row.names = FALSE)