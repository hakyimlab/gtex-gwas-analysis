##########################################################################################################
# Derived Rodrigo's code

getListOfPhenotypes <- function(table) {
  phenoLst <- query_exec(glue::glue("SELECT phenotype FROM {table$dataset_name}.{table$table_name} GROUP by phenotype"), table$project)
  phenoLst[[1]]
}
...
basicQuery <- function(table, columns=NULL, where=NULL, fake_submit=FALSE, max_pages = 1e4) {
  if (!is.null(columns)) { 
    columns <- paste(columns, collapse = ", ")
  } else {
    columns <- "*"
  }
  query_ <- "SELECT {columns} FROM {table$dataset_name}.{table$table_name}"
  if (!is.null(where)) {
    query_ <- paste0(query_, " WHERE {where}")
  }
  query <- glue::glue(query_)  
