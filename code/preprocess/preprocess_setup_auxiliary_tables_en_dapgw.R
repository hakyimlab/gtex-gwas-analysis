...
sp_count(predixcan_en_dapgw_tbl_eqtl, predixcan_en_dapgw_tbl_count_eqtl, "expression_b_en_dapgw.txt")
sp_count(predixcan_en_dapgw_tbl_sqtl, predixcan_en_dapgw_tbl_count_sqtl, "splicing_b_en_dapgw.txt")

sm_count <- function(sm_tbl, smc_tbl, store) {
  query <- glue::glue("SELECT count(pvalue) as n, phenotype FROM {sm_tbl$dataset_name}.{sm_tbl$table_name}
                      WHERE pvalue IS NOT NULL
                      GROUP BY phenotype")
  df <- query_exec(query, project = sm_tbl$project, max_pages = Inf) %>% suppressWarnings() %>%
    mutate(b=0.05/n) %>% select(phenotype, n, b)
  df %>% save_delim(fp_(store))

  bq_ <- bq_table(smc_tbl$project, smc_tbl$dataset_name, smc_tbl$table_name)
  bq_ <- bq_table_create(bq_, fields=as_bq_fields(df), friendly_name = "Number of results per trait")
  bq_table_upload(bq_, df)
}

sm_count(multixcan_en_dapgw_tbl_eqtl, multixcan_en_dapgw_tbl_count_eqtl, "expression_m_en_dapgw.txt")
sm_count(multixcan_en_dapgw_tbl_sqtl, multixcan_en_dapgw_tbl_count_sqtl, "splicing_m_en_dapgw.txt")
