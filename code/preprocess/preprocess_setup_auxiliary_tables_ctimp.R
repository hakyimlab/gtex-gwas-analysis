...
# FROM {predixcan_ctimp_tbl_eqtl$dataset_name}.{predixcan_ctimp_tbl_eqtl$table_name} as p
# INNER JOIN {gencode_all_annotation_tbl$dataset_name}.{gencode_all_annotation_tbl$table_name} AS a
# ON p.gene = a.gene_id
# GROUP BY p.gene, a.gene_type")
#     df <- query_exec(query, project = predixcan_ctimp_tbl_eqtl$project, max_pages = Inf) %>% suppressWarnings()
# })()

#We avoid bq_table_create because it converts `snake_case` to `camelCase`
#Create a table with bonferroni thresholds
(function(){
  ds <- bq_dataset(predixcan_ctimp_tbl_eqtl$project, predixcan_ctimp_tbl_eqtl$dataset_name)

  (function(){
    query <- glue::glue("SELECT count(pvalue) as n, phenotype FROM {predixcan_ctimp_tbl_eqtl$dataset_name}.{predixcan_ctimp_tbl_eqtl$table_name}
                      WHERE pvalue IS NOT NULL
                      GROUP BY phenotype")
    df <- query_exec(query, project = predixcan_ctimp_tbl_eqtl$project, max_pages = Inf) %>% suppressWarnings() %>%
      mutate(b=0.05/n) %>% select(phenotype, n, b)
    df %>% save_delim(fp_("expression_b_ctimp.txt"))

    bq_ <- bq_table(predixcan_ctimp_tbl_count_eqtl$project,predixcan_ctimp_tbl_count_eqtl$dataset_name, predixcan_ctimp_tbl_count_eqtl$table_name)
